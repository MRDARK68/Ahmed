...
window.buyWeapon = function (tier) {
    if (gameState.money >= WEAPONS[tier].cost && tier > gameState.currentWeaponIndex) {
        gameState.money -= WEAPONS[tier].cost; gameState.currentWeaponIndex = tier;
        ui.money.innerText = `Money: $${gameState.money}`;
        toggleShop(); // Update UI
        toggleShop(); // Re-open
    }
};
